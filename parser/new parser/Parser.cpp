...
void Parser::ParseDate()
{
        ...
        {   while (getline(vcd_file_, line_)){
                ...
                else if (line_.find(" ") != std::string::npos)
                {
                    simulation_date_ = line_.substr(1);
...
{
    if (line_.find("$version") != std::string::npos or line_.find("$Version") != std::string::npos)
        {   while (getline(vcd_file_, line_)){
                if (line_.find('\t') != std::string::npos){
                ...
                else if (line_.find("$end") != std::string::npos)
                {                    
                    version_ = line_.substr(8);
...
bool Parser::Parse(std::string vcdFilePath)
{
	...
	if (vcd_file_.is_open())
	{
		...
		while (getline(vcd_file_, line_))
		{
			...
			{
				vector<string> splittedLine ; 
				splittedLine = SplitBySpace(line_);
				Module module;
				...
				while (getline(vcd_file_, line_))
				{
					...
					else if(line_.find("$scope") !=std::string::npos)
					{
						splittedLine = SplitBySpace(line_);
						module.Name = splittedLine[2];
			...
			{
				char switchingActivitySignals[SwitchingActivitySignalsSize] = { '0', '1', 'z','b' };
				...
				while (getline(vcd_file_, line_))
				{
					...
					if ((line_[0] == '0') || (line_[0] == '1') || (line_[0] == 'z') || (line_[0] == 'b'))
					{
